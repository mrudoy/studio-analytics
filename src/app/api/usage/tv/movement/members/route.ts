...
import { getTvMovementMembers } from "@/lib/db/usage-store";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getTvMovementMembers({ group, from, to, fieldsOnly, page, perPage });
    return NextResponse.json(data);
  } catch (err) {
    console.error("[api/usage/tv/movement/members] Error:", err);
