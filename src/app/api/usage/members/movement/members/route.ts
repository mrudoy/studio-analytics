...
import { getMembersMovementMembers } from "@/lib/db/usage-store";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getMembersMovementMembers({ group, from, to, fieldsOnly, page, perPage });
    return NextResponse.json(data);
  } catch (err) {
    console.error("[api/usage/members/movement/members] Error:", err);
