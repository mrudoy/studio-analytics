    ...
    for (const row of rows) {
      await client.query(
        ...
        ON CONFLICT (email) DO UPDATE SET
          union_id = COALESCE(EXCLUDED.union_id, customers.union_id),
          ...
          twitter = COALESCE(EXCLUDED.twitter, customers.twitter),
          ...
          how_heard = COALESCE(EXCLUDED.how_heard, customers.how_heard),
          goals = COALESCE(EXCLUDED.goals, customers.goals),
          neighborhood = COALESCE(EXCLUDED.neighborhood, customers.neighborhood),
          inspiration = COALESCE(EXCLUDED.inspiration, customers.inspiration),
          practice_frequency = COALESCE(EXCLUDED.practice_frequency, customers.practice_frequency),
...
export async function getCustomerByEmail(email: string): Promise<CustomerProfile | null> {
  ...
  return {
    ...
    currentPaidAutoRenew: !!(Number(raw.current_paid_auto_renew)),
    ...
    inpersonRegistrations: Number(raw.inperson_registrations) || 0,
    livestreamRegistrations: Number(raw.livestream_registrations) || 0,
    replayRegistrations: Number(raw.replay_registrations) || 0,
    createdAt: raw.created_at as string,
    autoRenews: arRows.map((r: Record<string, unknown>) => ({
      planName: r.plan_name as string,
      planState: r.plan_state as string,
      planPrice: parseFloat(String(r.plan_price)) || 0,
      createdAt: r.created_at as string,
