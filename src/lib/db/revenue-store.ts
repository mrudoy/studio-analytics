...
export async function getRevenueForPeriod(periodStart: string, periodEnd: string): Promise<StoredRevenueRow[]> {
  ...
  return rows.map((r: Record<string, unknown>) => ({
    ...
    unionFees: Number(r.union_fees) || 0,
    stripeFees: Number(r.stripe_fees) || 0,
    otherFees: Number(r.other_fees) || 0,
    transfers: Number(r.transfers) || 0,
    refunded: Number(r.refunded) || 0,
    unionFeesRefunded: Number(r.union_fees_refunded) || 0,
...
/**
 * Get per-month sum of Union.fit retreat revenue categories.
 * Used to separate retreat revenue (mostly pass-through) from operating revenue.
 */
