...
- [ ] **Zip pipeline email resolution bug (pass-email cache)** — 50-90% of zip-imported registrations had empty email because passes.csv only contains active passes. Registrations from prior periods reference expired passes whose passId→membership→email join fails. Fix deployed (`41a5e47c`, `91b0612`): (1) persistent `pass_email_cache` table accumulates passId→email mappings across pipeline runs, (2) transformer uses cache as fallback, (3) `pass_id` column added to registrations for direct cache backfills, (4) backfill function uses cache + timestamp matching. Goal: raise resolution from ~45% to 95%+. Cache grows each pipeline run — first run populates from current passes, subsequent runs add expired passes. Needs monitoring over next few days.
