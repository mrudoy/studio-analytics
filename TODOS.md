...
- [ ] **Union.fit daily zip pipeline (PRIMARY)** — Union.fit sends a daily email to robot@skyting.com containing a download link. Clicking the link triggers a zip download with all source CSVs. This is the PRIMARY data source. Pipeline needs: poll Gmail → find Union.fit email → extract download URL from email body → fetch zip from URL → unzip → parse CSVs → upload to DB → update dashboard. First email has been received. This replaces the Playwright scraping approach. Playwright pipeline is now the FALLBACK for mid-day refreshes or when the email doesn't arrive.
