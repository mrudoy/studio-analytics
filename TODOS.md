...
- [ ] **Intro Week purchases not showing on dashboard** — 14 Intro Week orders placed Mon-Tue (2/23-2/24, $343.20 total) but dashboard shows none. Need to investigate: are these orders in the DB? Is the pipeline importing them? Is the dashboard filtering them out? Could be a purchased-vs-redeemed distinction or a pipeline freshness issue.
...
- [ ] **Remove redundant tooltip on annual revenue bar chart** — Hovering a bar shows a tooltip that just repeats the label already displayed above the bar. Remove the tooltip entirely since the data label is sufficient.
- [ ] **MRR card needs month labels** — "Monthly Recurring Revenue" card doesn't say which month it's for. Add current month AND previous month labels so user can compare (e.g. "February 2026" with Jan comparison).
