...
- [ ] **Churn change column shows wrong color (inverted polarity)** — In CategoryDetail cards (Members, Sky3, etc.), the "Churned last week" row shows green for +change and red for -change. This is backwards: more churn (+4) is BAD and should be red; less churn (-4) is GOOD and should be green. Same issue confirmed on both Members (+7 green) and Sky3 (+4 green). The "New last week" row colors are correct (more new = green, fewer new = red). Fix: invert color logic specifically for the churn row in the WoW comparison table.
