...
| Element | Rule |
|---------|------|
| `CartesianGrid` | Always present. Always `vertical={false}` (horizontal lines only) |
...
- Charts without `<CartesianGrid vertical={false} />` — every chart needs horizontal grid lines
