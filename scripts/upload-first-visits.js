...
if (!csvPath) {
  console.error('Usage: node scripts/upload-first-visits.js <csv-path>');
  process.exit(1);
}
...
async function main() {
  ...
  const before = await pool.query('SELECT COUNT(*) as count FROM first_visits');
  ...
  const after = await pool.query('SELECT COUNT(*) as count FROM first_visits');
